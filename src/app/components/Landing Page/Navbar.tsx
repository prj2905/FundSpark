    ...
    <nav role="navigation" className="w-full px-4 py-4 md:py-5">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        rounded-xl shadow-lg transition-all duration-300 animate-fade-in"
              >
                {categories.map((category) => (<button onClick={()=>{handleexplore(category)}} key={category}>
                  <li key={category} className="text-sm text-gray-800 hover:text-blue-600 cursor-pointer">
                    {category}
                  </li>
                  </button>
                ))}
          ...
          <Link href="#scroll" className="text-gray-700 hover:text-blue-500 font-medium">
            How it works
          </Link>
